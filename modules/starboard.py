...
class JumpView(discord.ui.View):
    def __init__(
        ...
        self.add_item(
            discord.ui.Button(
                url=url, label=label_name, style=discord.ButtonStyle.primary
            )
        )
...
class Starboard(core.Cog):
    ...
    async def remove_starer(self, user_id, message_id):
    ...
    async def handle_star(self, payload):
        ...
        bot_message = await starboard.send(HEADER_TEMPLATE.format(star, reaction_count, payload.channel_id, payload.channel_id))
